...
async function installEnvironment(index) {
  ...
  try {
    ...
    if (
      ...
      env.pytorch?.installCommands.length > 0
    ) {
    ...
    if (env.pip?.installCommands && env.pip?.installCommands.length > 0) {
      pipCommands = env.pip.installCommands.map((cmd) =>
        formatPipUrl(cmd, pipMirrorUrl)
      );
    }
...
async function createApp() {
  ...
  try {
    ...
    const newApp = {
      ...
      createdAt: props.isEditing
        ? store.appData.createdAt
